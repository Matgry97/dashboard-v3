  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.stats}>
        ...
        <div className={styles.stat}>
          <span className={styles.statValue}>{formatPace(run.distance, run.moving_time)}</span>
          <span className={styles.statLabel}>Pace</span>
        </div>
        {run.average_heartrate != null && (
          <div className={styles.stat}>
            <span className={styles.statValue}>{Math.round(run.average_heartrate)} bpm</span>
            <span className={styles.statLabel}>Avg HR</span>
          </div>
        )}
        {run.max_heartrate != null && (
          <div className={styles.stat}>
            <span className={styles.statValue}>{run.max_heartrate} bpm</span>
      ...
      {syncStatus === "error" && syncError && (
        <span className={styles.error}>{syncError}</span>
      )}
      <button
